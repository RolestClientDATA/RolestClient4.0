package minecraft.rolest.utils.drag;
...
public class Dragging {
    ...
    public final void onDraw(int mouseX, int mouseY, MainWindow res) {
        ...
        if (dragging) {
            ...
            if (xPos + (width / 2) >= res.getScaledWidth() / 2 - radius && xPos + (width / 2) <= res.getScaledWidth() / 2 + radius && (mouseX >= xPos)) {
                ...
                RenderUtility.drawRoundedRect(res.getScaledWidth() / 2, 0, 1, res.getScaledHeight(), 0, color);
    ...
    private void drawGridLines(MainWindow res) {
        ...
        int color = ((int) (alpha * 255) << 24) | 0xFFFFFF;
        if (showVerticalLine) {
            RenderUtility.drawRoundedRect(closestVerticalLine, 0, 1, res.scaledHeight(), 0, color);
        }

        if (showHorizontalLine) {
            RenderUtility.drawRoundedRect(0, closestHorizontalLine, res.scaledWidth(), 1, 0, color);
        }
    }
