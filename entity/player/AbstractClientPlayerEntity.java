...
import minecraft.rolest.utils.waveycapes.CapeHolder;
import minecraft.rolest.utils.waveycapes.sim.StickSimulation;
...
import minecraft.rolest.utils.render.GifUtils;
...
public abstract class AbstractClientPlayerEntity extends PlayerEntity implements CapeHolder
{
    ...
    public ResourceLocation getLocationCape() {
        ...
        // Check for local player or friend
        if ((this instanceof ClientPlayerEntity || FriendStorage.isFriend(this.getName().getString()))
                && !SelfDestruct.unhooked
                && (!moduleManager.getWorldTweaks().options.getValueByName("Маленький игрок").get() && moduleManager.getWorldTweaks().isState())
                && !(moduleManager.getNoRender().isState() && moduleManager.getNoRender().element.getValueByName("Плащ").get())) {
            GifUtils gifUtils = new GifUtils();
            return new ResourceLocation("rolka/images/cape69.png");
        }
    ...
    /**
     * Gets the special Elytra texture for the player.P
     */
