package minecraft.rolest.utils.math;
...
public class MathUtil implements IMinecraft {
    ...
    public static double atanInlining(double z, double x) {
        if (x == 0.0 && z == 0.0) {
        ...
        boolean zNegative = z < 0.0;
        boolean xNegative = x < 0.0;

        // Используем аппроксимацию для atan2
        if (absX >= absZ) {
            double ratio = absZ / absX;
            angle = ratio * (0.7853981633974483 - 0.08074551218828078 * ratio * ratio); // Апроксимация для малых углов
            angle = xNegative ? Math.PI - angle : angle;
        } else {
            double ratio = absX / absZ;
            angle = 1.5707963267948966 - ratio * (0.7853981633974483 - 0.08074551218828078 * ratio * ratio);
            angle = zNegative ? -angle : angle;
        }

        // Коррекция знака и учет квадрантов
        if (xNegative && zNegative) {
            angle -= Math.PI;
