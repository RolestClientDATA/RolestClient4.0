...
public class AutoBuyUI extends Screen implements IMinecraft {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtility.drawRoundedRect(x, y, width, height, new Vector4f(7, 7, 7, 7),
                ColorUtils.rgba(17, 17, 17, 255));


        RenderUtility.drawRoundedRect(x + 10, y + height - 60, width - 20, 50, 4, ColorUtils.rgba(23, 23, 23, 255));
        Fonts.sfuy.drawCenteredText(matrixStack, selecting ? "Назад" : "Добавить", x + width / 2f, y + height - 30 - 10, MathUtil.isHovered(mouseX, mouseY, x + 10, y + height - 60, width - 20, 50) ? -1 : ColorUtils.rgba(255, 255, 255, 128), 10);
        if (selecting) {
            ...
            RenderUtility.drawRoundedRect(x + 5, itemY, width - 10, 28, 3, ColorUtils.rgba(23, 23, 23, 255));
            RenderUtility.drawRectW(x + width - 100, itemY, 0.5f, 28, ColorUtils.rgb(17, 17, 17));
            {
            ...
            Fonts.sfuy.drawText(matrixStack, textToDraw + (searching ? System.currentTimeMillis() % 1000 > 500 ? "_" : "" : ""), x + 10, itemY + 28 / 2f - 7 / 2f, -1, 7);
            itemY += 30;
            ...
            for (AutoBuy buy : Rol.getInstance().getAutoBuyHandler().items) {


                RenderUtility.drawRoundedRect(x + 10 + buyX, y + 10 + buyY, widthPanel, 50, 3, ColorUtils.rgba(23, 23, 23, 255));
                RenderUtility.drawRoundedRect(x + 15 + buyX, y + 15 + buyY, 20, 20, 3, ColorUtils.rgba(30, 30, 30, 255));
                ...
                Fonts.sfuy.drawText(matrixStack, buy.getItem().getName().getString(), (int) (x + 37 + buyX), y + 21 + buyY, -1, 7, 0.05f);
                if (Fonts.sfuy.getWidth(buy.getItem().getName().getString(), 7, 0.05f) + 37 >= widthPanel) {
                    RenderUtility.drawRectVerticalW(x + buyX + widthPanel, y + 21f + buyY, 10, 7, ColorUtils.rgba(23, 23, 23, 255), ColorUtils.rgba(23, 23, 23, 0));
                }

                Fonts.sfuy.drawText(matrixStack, "Цена: ", (int) (x + 13 + buyX), y + 40 + buyY, ColorUtils.rgba(255, 255, 255, 128), 5, 0.05f);
                Fonts.sfuy.drawText(matrixStack, String.valueOf(buy.getPrice()), (int) (x + 13 + buyX) + Fonts.sfuy.getWidth("Цена: ", 5, 0.05f), y + 40 + buyY, ColorUtils.rgba(64, 255, 64, 128), 5, 0.05f);
                float off = 0;
                if (!buy.getEnchanments().isEmpty()) {
                    Fonts.sfuy.drawText(matrixStack, "Чары: ", (int) (x + 13 + buyX), y + 40 + buyY + 10, ColorUtils.rgba(255, 255, 255, 128), 5, 0.05f);
                    for (Enchantment enchantment : buy.getEnchanments().keySet()) {
                        ...
                        Fonts.sfuy.drawText(matrixStack, enchText, (int) (x + 13 + buyX) + Fonts.sfuy.getWidth("Чары: ", 5, 0.05f) + off, y + 40 + buyY + 10, ColorUtils.rgba(255, 255, 255, 128), 5, 0.05f);
                        off += Fonts.sfuy.getWidth(enchText, 5, 0.05f) + 1;
                    }
                }
                if (buy.isItems() || buy.isDon()) {
                    Fonts.sfuy.drawText(matrixStack, "Только с предметами", (int) (x + 13 + buyX), y + 40 + buyY + 10, ColorUtils.rgba(255, 0, 0, 128), 5, 0.05f);
                } else if (buy.isFake()) {
                    Fonts.sfuy.drawText(matrixStack, "Проверяется на фейк предмет", (int) (x + 13 + buyX), y + 40 + buyY + 10, ColorUtils.rgba(255, 0, 0, 128), 5, 0.05f);
        ...
        if (selecting) {
            ...
            for (Item item : Registry.ITEM) {
                ...
                if (MathUtil.isHovered((float) mouseX, (float) mouseY, itemX, itemY, 15, 15) && MathUtil.isHovered((float) mouseX, (float) mouseY, x, y, width, height - 65)) {
                    ...
                    for (AutoBuy a : Rol.getInstance().getAutoBuyHandler().items) {
                        if (a.getItem() == item) {
                            ignore = true;
                            break;
                        }
            ...
            for (AutoBuy buy : Rol.getInstance().getAutoBuyHandler().items) {
                if (MathUtil.isHovered((float) mouseX, (float) mouseY, x + 10 + buyX, y + 10 + buyY, widthPanel, 50) && button == 1) {
                    Rol.getInstance().getAutoBuyHandler().items.remove(buy);
