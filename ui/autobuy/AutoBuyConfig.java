...
public class AutoBuyConfig implements IMinecraft {
    ...
    private static final File file = new File(mc.gameDir, "\\saves\\files\\other\\autobuy.rolka");
    ...
    public static void updateFile() {
        ...
        for (AutoBuy autoBuy : Rol.getInstance().getAutoBuyHandler().items) {
    ...
    private void readBuyConfig() throws FileNotFoundException {
        ...
        if (jsonObject.has("autoBuys")) {
            for (JsonElement element : jsonObject.get("autoBuys").getAsJsonArray()) {
                ...
                Rol.getInstance().getAutoBuyHandler().items.add(new AutoBuy(item, price,autoBuyObject.get("count").getAsInt(), enchantments,
