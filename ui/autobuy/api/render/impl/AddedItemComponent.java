package minecraft.rolest.ui.autobuy.api.render.impl;
...
public class AddedItemComponent extends Component {
    ...
    public void render(MatrixStack stack, int mouseX, int mouseY) {
        ...
        float tx = 0;
        float fak = 0;
        for (IItem addingItem : Rol.getInstance().getItemStorage().getItems()) {
        ...
        for (IItem addingItem : Rol.getInstance().getItemStorage().getItems()) {
            ...
            RenderUtility.drawRoundedRect(x + currentX, y + currentY, panelWidth, panelHeight, 3, ColorUtils.rgba(23, 23, 23, 255));
            drawIcon(item, currentX, currentY, padding, iconWidth, iconHeight, iconPadding,addingItem);


            float fontHeight = Fonts.sfuy.getHeight(7);
            Fonts.sfuy.drawText(stack, displayName, x + currentX + iconWidth + (padding * 2), y + currentY + padding + (iconHeight / 2) - (fontHeight / 2), -1, 7);
            Fonts.sfuy.drawText(stack, "Цена: " + addingItem.getPrice() + "$", x + currentX + (padding), y + currentY + iconHeight + (padding * 2), -1, 6);
            Fonts.sfuy.drawText(stack, "Кол-во: " + addingItem.getQuantity() + " шт.", x + currentX + (padding), y + currentY + iconHeight + (padding * 4), -1, 6);
            Fonts.sfuy.drawText(stack, "Чары" + renderEnchantmentsText(addingItem.getEnchantments()), x + currentX + (padding), y + currentY + iconHeight + (padding * 6), -1, 6);
    ...
    private void drawIcon(Item item, float currentX, float currentY, float padding, float iconWidth, float iconHeight, float iconPadding, IItem it1) {
        RenderUtility.drawRoundedRect(x + currentX + padding, y + currentY + padding, iconWidth, iconHeight, 3, ColorUtils.rgba(30, 30, 30, 255));
        ...
        if (mouseButton == 1) {
            ...
            for (IItem addingItem : Rol.getInstance().getItemStorage().getItems()) {
                if (isHovered(mouseX, mouseY, x + currentX, y + currentY, panelWidth, panelHeight)) {
                    Rol.getInstance().getItemStorage().getItems().remove(addingItem);
                }
                currentX += panelWidth + (padding * 2.6f);
                if (x + currentX + panelWidth >= x + width) {
                    currentX = 0;
