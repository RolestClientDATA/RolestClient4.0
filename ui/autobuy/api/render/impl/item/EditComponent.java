package minecraft.rolest.ui.autobuy.api.render.impl.item;

import com.mojang.blaze3d.matrix.MatrixStack;

import minecraft.rolest.Rol;
import minecraft.rolest.ui.autobuy.api.render.impl.AddedItemComponent;
import minecraft.rolest.ui.autobuy.api.render.impl.AllItemComponent;
import minecraft.rolest.ui.autobuy.api.render.impl.Component;
import minecraft.rolest.utils.client.IMinecraft;
import minecraft.rolest.utils.components.ButtonComponent;
import minecraft.rolest.utils.components.FieldComponent;
import minecraft.rolest.utils.components.SliderComponent;
...
public class EditComponent extends Component implements IMinecraft {
    ...
    private final ButtonComponent add = new ButtonComponent(0, 0, 0, 0, "Добавить", () -> {
        if (!price.get().isEmpty()) {
            ...
            Rol.getInstance().getItemStorage().addItem(stack.getItem(), Integer.parseInt(price.get()), Integer.parseInt(count.fieldComponent.get()), Integer.parseInt(damage.fieldComponent.get()), enchantmentWidget.get());
    ...
    public void render(MatrixStack stack, int mouseX, int mouseY) {
        ...
        float width = 100;
        Fonts.sfuy.drawText(stack, TextFormatting.getTextWithoutFormattingCodes(this.stack.getDisplayName().getString()), x + 24, y + 9f, -1, 6, 0.05f);
        if (Fonts.sfuy.getWidth(TextFormatting.getTextWithoutFormattingCodes(this.stack.getDisplayName().getString()), 6, 0.05f) + 24 > width) {
            RenderUtility.drawRectVerticalW(x + width - 10f, y + 9f, 10, 6, ColorUtils.rgba(17, 17, 17, 255), ColorUtils.rgba(17, 17, 17, 0));
        }
