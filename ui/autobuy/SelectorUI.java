package minecraft.rolest.ui.autobuy;
...
public class SelectorUI implements IMinecraft {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtility.drawShadow(x, y, width, height, 10,
                ColorUtils.rgba(17, 17, 17, 128));
        Scissor.push();
        Scissor.setFromComponentCoordinates(x, y, width, height);
        RenderUtility.drawRoundedRect(x, y, 100, 100, 4, ColorUtils.rgba(17, 17, 17, 255));
        mc.getItemRenderer().renderItemAndEffectIntoGUI(instance, (int) x + 5, (int) y + 5);
        Fonts.sfuy.drawText(matrixStack, TextFormatting.getTextWithoutFormattingCodes(selected.getName().getString()), x + 24, y + 9f, -1, 6, 0.05f);
        RenderUtility.drawShadow(x + 5, y + 100 - 20, 100 - 10, 15, 10,
                ColorUtils.rgba(27, 27, 27, 255));

        if (Fonts.sfuy.getWidth(TextFormatting.getTextWithoutFormattingCodes(selected.getName().getString()), 6, 0.05f) + 24 > width) {
            RenderUtility.drawRectVerticalW(x + width - 10, y + 9f, 10, 6, ColorUtils.rgba(17, 17, 17, 255), ColorUtils.rgba(17, 17, 17, 0));
        }

        RenderUtility.drawRoundedRect(x + 5, y + 100 - 20, 100 - 10, 15, 3, ColorUtils.rgba(27, 27, 27, 255));
        Fonts.sfuy.drawCenteredText(matrixStack, "Добавить", x + width / 2f, y + height - 20 + 7 / 2F, -1, 7, 0.05F);
        Fonts.sfuy.drawText(matrixStack, "Цена", x + 8, y + 23, -1, 6, 0.03f);
        RenderUtility.drawRoundedRect(x + 7, y + 30, 50, 11, 2, ColorUtils.rgba(27, 27, 27, 255));
        Fonts.sfuy.drawText(matrixStack, price + (priceClicked ? System.currentTimeMillis() % 1000 > 500 ? "_" : "" : ""), x + 10, y + 33, -1, 5, 0.03f);

        if (Block.getBlockFromItem(selected) instanceof ShulkerBoxBlock b) {
            Fonts.sfuy.drawText(matrixStack, "Есть донат предметы?", x + 8, y + 23 + 20, -1, 6, 0.03f);
            RenderUtility.drawRoundedRect(x + 8 - 2, y + 23 + 28 - 2 + 0.5f, Fonts.sfuy.getWidth(don ? "Да" : "Нет", 6, 0.03f) + 4.5f, 5 + 4, 2, ColorUtils.rgba(25, 25, 25, 255));
            Fonts.sfuy.drawText(matrixStack, don ? "Да" : "Нет", x + 8, y + 23 + 28, -1, 6, 0.03f);

            Fonts.sfuy.drawText(matrixStack, "Есть предметы?", x + 8, y + 23 + 20 + 17, -1, 6, 0.03f);
            RenderUtility.drawRoundedRect(x + 8 - 2, y + 23 + 28 - 2 + 0.5f + 17, Fonts.sfuy.getWidth(items ? "Да" : "Нет", 6, 0.03f) + 4.5f, 5 + 4, 2, ColorUtils.rgba(25, 25, 25, 255));
            Fonts.sfuy.drawText(matrixStack, items ? "Да" : "Нет", x + 8, y + 23 + 28 + 17, -1, 6, 0.03f);
        } else {
            Fonts.sfuy.drawText(matrixStack, "Количество", x + 8, y + 23 + 20, -1, 6, 0.03f);
            RenderUtility.drawRoundedRect(x + 7, y + 30 + 20, 20, 8, 2, ColorUtils.rgba(27, 27, 27, 255));
            Fonts.sfuy.drawText(matrixStack, count + (countClicked ? System.currentTimeMillis() % 1000 > 500 ? "_" : "" : ""), x + 10, y + 33 + 19, -1, 5, 0.03f);

            Fonts.sfuy.drawText(matrixStack, "Проверять на фейк", x + 8, y + 23 + 20 + 17, -1, 6, 0.03f);
            RenderUtility.drawRoundedRect(x + 8 - 2, y + 23 + 28 - 2 + 0.5f + 17, Fonts.sfuy.getWidth(fake ? "Да" : "Нет", 6, 0.03f) + 4.5f, 5 + 4, 2, ColorUtils.rgba(25, 25, 25, 255));
            Fonts.sfuy.drawText(matrixStack, fake ? "Да" : "Нет", x + 8, y + 23 + 28 + 17, -1, 6, 0.03f);
        }
        GlStateManager.pushMatrix();
        glTranslatef(x + width - 12 + 12 / 2f, y + 2 + 12 / 2f, 0);
        glRotatef(45, 0, 0, 1);
        glTranslatef(-(x + width - 12 + 12 / 2f), -(y + 2 + 12 / 2f), 0);
        Fonts.sfuy.drawText(matrixStack, "+", x + width - 12, y + 2, ColorUtils.rgba(255, 255, 255, 128), 12f);
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (Block.getBlockFromItem(selected) instanceof ShulkerBoxBlock b) {
            if (MathUtil.isHovered((float) mouseX, (float) mouseY, x + 8 - 2, y + 23 + 28 - 2 + 0.5f + 17, Fonts.sfuy.getWidth(items ? "Да" : "Нет", 6, 0.03f) + 4, 5 + 4)) {
                items = !items;

            }
            if (MathUtil.isHovered((float) mouseX, (float) mouseY, x + 8 - 2, y + 23 + 28 - 2 + 0.5f, Fonts.sfuy.getWidth(ench ? "Да" : "Нет", 6, 0.03f) + 4, 5 + 4)) {
                don = !don;
            }
        } else {
            if (MathUtil.isHovered((float) mouseX, (float) mouseY, x + 8 - 2, y + 23 + 28 - 2 + 0.5f, Fonts.sfuy.getWidth(ench ? "Да" : "Нет", 6, 0.03f) + 4, 5 + 4)) {
            ...
            if (MathUtil.isHovered((float) mouseX, (float) mouseY, x + 8 - 2, y + 23 + 28 - 2 + 0.5f + 17, Fonts.sfuy.getWidth(fake ? "Да" : "Нет", 6, 0.03f) + 4, 5 + 4)) {
                fake = !fake;
            }
        }
        // Добавил предмет
