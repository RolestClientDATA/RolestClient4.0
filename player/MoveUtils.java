...
public class MoveUtils implements IMinecraft {
    ...
    public static void handleAirJumpMotion() {
        if (mc.player == null || mc.world == null) return;

        // Плавное гашение скорости после прыжка
        if (Math.abs(mc.player.getMotion().y) > 0.1) {
            mc.player.setMotion(
                    mc.player.getMotion().x * 0.985f,
                    mc.player.getMotion().y * 0.92f,
                    mc.player.getMotion().z * 0.985f
            );
        }

        // Дополнительная стабилизация
        if (!mc.player.isOnGround() && mc.player.getMotion().y < -0.1) {
            mc.player.setMotion(
                    mc.player.getMotion().x,
                    mc.player.getMotion().y * 0.96f,
    ...
    public static double[] getSpeed(double speed) {
        HitAura hitAura = Rol.getInstance().getModuleManager().getHitAura();
