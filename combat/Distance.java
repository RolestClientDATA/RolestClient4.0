...
import minecraft.rolest.events.EventRender3D;
...
@ModuleRegister(name = "TPInfluence", category = Category.Combat,desc ="хуета на тесте")
public class Distance extends Module {
    ...
    public void onRender(EventRender3D e) {
        if (render.get() && Rol.getInstance().getModuleManager().getHitAura().getTarget() != null && Rol.getInstance().getModuleManager().getHitAura().isState()) {
            ...
            RenderUtils.drawBox(new AxisAlignedBB(lastHandledVec.x - half, lastHandledVec.y, lastHandledVec.z - half, lastHandledVec.x + half, lastHandledVec.y + mc.player.getHeight(), lastHandledVec.z + half), ColorUtils.setAlpha(-1, 130));
    ...
    public void hitAuraTPPRe(LivingEntity target) {
        if (isState() && !Rol.getInstance().getModuleManager().getFreeCam().isState()) teleportActionOfActionType(true, targetWhitePos(target, Rol.getInstance().getModuleManager().getHitAura().attackDistance()), mode.get());
    }

    public void hitAuraTPPost(LivingEntity target) {
        if (isState() && !Rol.getInstance().getModuleManager().getFreeCam().isState()) teleportActionOfActionType(false, targetWhitePos(target, Rol.getInstance().getModuleManager().getHitAura().attackDistance()), mode.get());
    }
    ...
    public Vector3d targetWhitePos(LivingEntity target, double distanceMin) {
        ...
        if (yDst > distanceMin) {
            ...
            for (tempAppend = 0.0; tempAppend < appendY; tempAppend += 0.1) {
                ...
                if (!(tempAppend > 0.0) || Distance.mc.world.getCollisionShapes(target, aabb).toList().isEmpty()) continue;
                tempAppend -= 0.1;
                break;
            }
